...
def bin_Initialization():
    ...
    algorithm_parameters = {}
    num_Chromosomes = int(input("Enter the number of Chromosomes: "))
    algorithm_parameters["num_Chromosomes"] = num_Chromosomes #N
    num_Bits = int(input("Enter the number of bits (L) : "))
    algorithm_parameters["num_Bits"] = num_Bits #L
    num_Genes = int(input("* L should be devidable by M * Enter the number of Genes (M): "))
    algorithm_parameters["num_Genes"] = num_Genes #M
    num_Generations = int(input("Enter the number of Generations: "))
    algorithm_parameters["num_Generations"] = num_Generations
    Pc = round(random.uniform(0.7, 0.9), 3)
    print("The Possibality of Crossover is: ", Pc)
    algorithm_parameters["Pc"] = Pc
    Pm = round(random.uniform(0.01, 0.1), 3)
    print("The Possibality of Mutation is: ", Pm)
...
def bin_Create_First_Generation(algorithm_parameters):

    population = []

    for i in range(algorithm_parameters.get("num_Chromosomes")):
        line = ""
        for j in range(algorithm_parameters.get("num_Bits")):
